...
?>
        <!-- Live Region for screen readers, It notifys the user about the status of any changes -->
        <!-- When i neeed to notify the user i just update its content, Visual users won't see it -->
        <div id="woohsn-status" class="screen-reader-text" aria-live="polite"></div>

        <div class="woohsn-meta-box">
            <p>
                <label for="woohsn_code">
                    <strong>
                        <?php esc_html_e('HSN Code:', 'woohsn'); ?>
                    </strong>
                </label>
                <input type="text" id="woohsn_code" name="woohsn_code" value="<?php echo esc_attr($hsn_code); ?>"
                    aria-describedby="woohsn-code-description" class="widefat" placeholder="<?php esc_attr_e('Enter HSN code', 'woohsn'); ?>" />
                <button type="button" id="woohsn-suggest-btn" class="button button-small"
                    aria-controls="woohsn-suggestions"
                    aria-expanded="false">
            ...
            <div
                id="woohsn-suggestions"
                role="region"
                aria-live="polite"
                aria-label="HSN code suggestions"
                hidden>
                <p><strong><?php esc_html_e('Suggested HSN Codes:', 'woohsn'); ?></strong></p>
                <ul id="woohsn-suggestions-list" role="listbox"></ul>
            </div>

            <p>
                <label for="woohsn_enable_custom_gst">
                    <input type="checkbox" id="woohsn_enable_custom_gst" name="woohsn_enable_custom_gst"
                        aria-describedby="woohsn-enable-custom-gst-description" value="yes" <?php checked($enable_custom_gst, 'yes'); ?>
                        aria-controls="woohsn-custom-gst-field" aria-expanded="<?= $enable_custom_gst === 'yes' ? 'true' : 'false' ?>" />
            ...
            <p id="woohsn-custom-gst-field" style="<?php echo $enable_custom_gst !== 'yes' ? 'display: none;' : ''; ?>">
                ...
                <input type="number" id="woohsn_custom_gst_rate" name="woohsn_custom_gst_rate"
                    aria-describedby="woohsn-custom-gst-rate-description" value="<?php echo esc_attr($custom_gst_rate); ?>" step="0.01" min="0" max="100" class="widefat" />
            </p>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function($) {
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                $('.woohsn-suggestion').click(function() {
                                    ...
                                    $('#woohsn-suggest-btn').attr('aria-expanded', 'false');

                                    // Update live region
                                    $('#woohsn-status').text('HSN code ' + hsnCode + ' selected.');

                                    // Optional: mark selected suggestion for screen readers
                                    $('.woohsn-suggestion').attr('aria-selected', 'false'); // reset others
                ...
                // Handle keyboard navigation for more accessibility
                $('#woohsn-suggestions-list').on('keydown', '.woohsn-suggestion', function(e) {
                    var $current = $(this);
                    if (e.key === 'ArrowDown') {
                        e.preventDefault();
                        $current.next().focus();
